...
variable "ext-network-0001-cidr-ipv4" {
  type = string
  description = "ext-network-0001 IPv4 CIDR"
}
variable "ext-network-0001-cidr-ipv6" {
  type = string
  description = "ext-network-0001 IPv6 CIDR"
}
...
resource "openstack_networking_subnet_v2" "ext-network-0001-subnet-0001" {
  ...
  cidr = cidrsubnet(var.ext-network-0001-ipv4, 8, 1)
  ip_version = 4
  gateway_ip = cidrhost(cidrsubnet(var.ext-network-0001-ipv4, 8, 1), 1)
  enable_dhcp = false
}
...
resource "openstack_networking_subnet_v2" "ext-network-0001-subnet-0002" {
  ...
  network_id = openstack_networking_network_v2.ext-network-0001.id
  cidr = cidrsubnet(var.ext-network-0001-ipv6, 16, 1)
  ip_version = 6
  ipv6_address_mode = "slaac"
  ipv6_ra_mode = "slaac"
  gateway_ip = cidrhost(cidrsubnet(var.ext-network-0001-ipv6, 16, 1), 1)
  enable_dhcp = false
}
